...
export function getCurrentDate() {
  ...
  return month + '月' + day + '日' + weekDay + '·' + lunar
}

/**
 * 启动时钟定时器
 * @param {Function} callback 每次tick的回调
 * @param {number} interval 间隔(毫秒), 默认1000
 */
export function startClock(callback, interval) {
  callback()
...
// 农历数据 1900-2100 年，每个值编码了该年各月大小月信息及闰月
const lunarInfo = [
...
function getLunarDate(date) {
  ...
  const yearStr = tianGan[ganIdx] + diZhi[zhiIdx] + '年'

  // 月日
  const monthStr = (isLeap ? '闰' : '') + lunarMonthNames[lunarMonth - 1] + '月'
  const dayStr = lunarDayNames[lunarDay - 1]
